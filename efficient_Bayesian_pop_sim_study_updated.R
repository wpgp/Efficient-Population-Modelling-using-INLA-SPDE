...
out_path <- paste0(path, "/sim_study/output_revised") # same data source variance
...
spat.autocor <- c(0.015, 0.150, 1.500)#spatial autocorrelation variance

#source_re <- c(0.01, 0.1, 1)#variance of data source random effects
source_var <- list(same = rep(0.001, 5), 
                   diff = c(0.0015, 0.001, 0.0018, 0.0013, 0.001))#variance of data source random effects
...
{ 
  ...
  #--generate data type random effects 
    ...
    for(j in 1:length(cover))###----Do the following for each coverage percentage 
    {
      ...
      pp <- cover[j]
      opts <- sample(nrow(coord), pp*nrow(coord))
      opts <- sort(opts)
      dim(dat <-  data.frame(datam[opts,])) #---Locations with observations
      
      ##--------------
      coord8 <- cbind(dat$lon, dat$lat)
      dim(coord8)
      ...
      # Spatial 
      form1 <- y ~ -1 + Intercept + x2 + x3 + x4 + x5 + x6 + 
        f(eps, model='iid') +
        f(source1, model='iid') +
      ...
      gproj1 <- inla.mesh.projector(mesh,  dims = c(300, 300))
      
      col <- viridis(100)
      
      g.mean1 <- inla.mesh.project(gproj1, mod1$summary.random$spatial.field$mean)
      g.sd1 <- inla.mesh.project(gproj1, mod1$summary.random$spatial.field$sd)
      
      
      grid.arrange(levelplot(g.mean1, scales=list(draw=F), xlab='', ylab='', cex.lab=2, main='Mean',col.regions = col),
                   levelplot(g.sd1, scal=list(draw=F), xla='', yla='', main='SD' ,col.regions = col), nrow=1)
  ...
  # extract posterior values from the non spatial model      
      ...
      pred1<- exp(mod1$summary.linear.predictor[index ,"mean"]) #--predicted mean count
 ...
 # extract posterior values from the non spatial model
      pred2<- exp(mod2$summary.linear.predictor$mean) #--predicted mean count
      ...
      system.time(str(sim.spat <- sim_spatial(mod1,dat.pred,Aprd, run))) # spatial
      system.time(str(sim.nspat <- sim_nonspatial(mod2,dat.pred, run))) # non-spatial
      
      dat1 <- sim.spat$est_data
      dat1$method <- rep("base", nrow(dat1))
      dat2 <- sim.nspat$est_data
...
#---------------------------------------------------------------------------------------
out_path <- paste0(path, "/sim_study/output_revised1") # different data source variance
cover <- seq(0.1,1, by=0.1)
spat.autocor <- c (0.001, 0.01, 0.1)
source_var <- list(same = rep(0.001, 5), 
                   diff = c(0.0015, 0.0021, 0.0008, 0.0025, 0.0095))#variance of data source random effects
...
{ 
  ...
  for(j in 1:length(cover))###----Do the following for each coverage percentage 
  {
    ...
    dat.pred$source_re <- data_type_re(dat.pred, 
                                       mod1$summary.random$source1$mean,
                                       dat.pred$source)
    ...
    dat1 <- sim.spat$est_data
    dat1$method <- rep("proposed", nrow(dat1))
    dat2 <- sim.nspat$est_data
    dat2$method <- rep("base", nrow(dat2))
...
#=======================================================
#Run posterior plots for same data source variance models
# ==========================================================
# Specify key data paths
path <- "//worldpop.files.soton.ac.uk/Worldpop/Projects/WP517763_GRID3/Working/CMR/Chris_N/paper1"
shp_path <- paste0(path, "/application/data/Input_Settlement_Boundaries")
out_path <- paste0(path, "/sim_study/output_revised")

cover <- c(1, 0.8, 0.6, 0.4, 0.2, 0.05) #--percentage coverage of the observations
spat.autocor <- c(0.015, 0.15, 1.5)#spatial autocorrelation variance
...
{ 
  ...
  for(j in 1:length(cover))###----Do the following for each coverage percentage 
  {
    ...
    mod_fit$method <- c("spatial", "nspatial")
    metsb1[[j]] <- mod_fit
  }
  metsb2[[b]] <- metsb1 
}
...
rrmse <-  ggpar(plot_rmse, xlab="Spatial Variance", ylab="Root Mean Square Error (RMSE)",
                legend = "top", legend.title = "Survey Coverage (%)",size=20,
                font.legend=c(20),
