...
pred.data2 <- pred.data %>% dplyr::filter(pred<3000)
plot_cval4 <- ggboxplot(pred.data2, y = "pred",
          #add = "reg.line",                         # Add regression line
          facet.by = "data",
          #conf.int = TRUE,                          # Add confidence interval
          color = "Fold", 
...
plot_cval4 <- pred.data %>% dplyr::filter(pred<5000) %>%
...
rcval5 <-  ggpar(barp, xlab="Population Estimate", ylab="Region",
                 legend = "right", 
                 #legend.title = "Dataset",size=20,
...
library(viridis)
brks <- seq(0,800, by=100)
zlim=c(0,800)
plot(cmr_grd.adj,
     legend=T, col = colorRampPalette(c("dark green", "orange", "white"))(255),
     asp = NA, cex.axis=1.4, axes=F,
     gridded=T, breaks = brks, zlim=zlim)
plot(x_lonlat, add=T, legend=F)
plot(shpp,add=T, col="black", lwd=2)
plot(cmr_grd.adj,
     legend=F, col = colorRampPalette(c("dark green", "orange", "white"))(255),
     asp = NA, 
...
# add Douala
ext_dou <- extent(9.6, 9.9, 3.9, 4.2)/2
plot(ext_dou , add=TRUE, col='red', lwd=2)

# add Yaounde
ext_ynde <- extent(11.4, 11.7, 3.7, 4)/2
plot(ext_ynde , add=TRUE, col='red', lwd=2)
...
brks2 <- seq(0,1.5, by=0.25)
zlim2=c(0,1.5)
plot(rcv,
     legend=T, col = colorRampPalette(c("green", "orange", "red"))(255),
     asp = NA, cex.axis=1.4, axes=F,
     gridded=T, breaks = brks2, zlim=zlim2)
plot(x_lonlat2, add=T, legend=F)
plot(shpp,add=T, col="black", lwd=2)
plot(rcv,
     legend=F, col = colorRampPalette(c("green", "orange", "red"))(255),
     asp = NA, 
     cex.axis=1.4, add=T, breaks = brks2, zlim=zlim2)
